...
/**
 * @author: ANAYED HOSSAIN ESHSAN - 23202807;
 *
...
public class BlockPuzzle {
    ...
    void getInputAndPlacePuzzle(char[][] map, char[][][] puzzles) {
        ...
        while(true) {
            System.out.print("Enter the puzzle number: ");
            int puzzleNumber = in.nextInt();
            in.nextLine();
            if ((puzzleNumber < 0) || (puzzleNumber > 2) || puzzles[puzzleNumber] == null) {
                System.out.println("Invalid puzzle Index!");
                continue;
            }
            System.out.print("Enter the position you want to place the puzzle (e.g. a1): ");
            ...
            if (canPlace(map, puzzles[puzzleNumber], row, col)){
                place(map, puzzles[puzzleNumber], row, col);
                puzzles[puzzleNumber] = null;
                break;
            } else {
                System.out.println("Invalid index to place the puzzle!");
            }
    ...
    void randomPuzzleIfNeeded(char[][][] puzzles) {
        ...
        if (allisNull){
            for (int j = 0; j < ROUND; j++){
                int randomInt = ThreadLocalRandom.current().nextInt(0, PUZZLES.length);
                puzzles[j] = PUZZLES[randomInt];
            }
        }
    }
    ...
    boolean canPlace(char[][] map, char[][] puzzle, int r, int c) {
        if (r < 0 || c < 0 || r + puzzle.length > ROW || c + puzzle[0].length > COL) {
            return false;
        }

        for (int i = 0; i < puzzle.length; i++) {
            for (int j = 0; j < puzzle[i].length; j++) {
                if (puzzle[i][j] != EMPTY && map[r + i][c + j] != EMPTY) {
                    return false;
                }
            }
        }
    ...
    boolean validateInput(String input) {
        char rowN = input.charAt(0);
        char colN = input.charAt(1);

        return input.length() == 2 && rowN >= 'a' && rowN <= 'z' && colN >= '0' && colN <= '9';
    }
    ...
    int cancelPuzzles(char[][] map) {
        ...
        boolean[] emptyRow = new boolean[ROW];
        boolean[] emptyCol = new boolean[COL];
